...
struct ClearGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://clearwatg.top/get")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            ClearEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    ClearGameInitialView()
}
